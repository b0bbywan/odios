...
EOF
    ...
    case "${ACTION}" in
      ...
      upgrade-from-image-progress)
          # Live embedded upgrade with --progress; the baseline (built from this
          # PR) ships an odio-upgrade that knows --progress. Capture stdout to
          # assert the ODIO_PROGRESS events install.sh emits via the callback.
          PROGRESS_LOG=/tmp/odio-upgrade-progress.log
          run_odio_upgrade_embedded "${TARGET}" live --progress > "${PROGRESS_LOG}" 2>&1 \
            || { cat "${PROGRESS_LOG}"; echo "odio-upgrade failed" >&2; exit 1; }
          cat "${PROGRESS_LOG}"
