#!/usr/bin/env python3
"""Assert state.json matches the shape odio-upgrade reads.
...
# Mirrors odio_upgrade.find_state's cascade. The /var/cache fallback covers
# the systemctl path on PR runs that haven't been tagged yet — odio.love
# still advertises the previous stable, so the upgrade lands on a release
# whose playbook predates the /var/lib migration. Drops out of use once
# the next stable ships with the new layout.
STATE_PATHS = ("/var/lib/odio/state.json", "/var/cache/odio/state.json")
...
def main() -> int:
    if len(sys.argv) != 2:
        ...
        return 2
    tag = sys.argv[1]
    state_path = next((p for p in STATE_PATHS if os.path.isfile(p)), None)
    assert state_path, f"no state.json found in any of {STATE_PATHS}"
