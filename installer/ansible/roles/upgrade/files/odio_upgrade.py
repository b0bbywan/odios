...
Subcommands:
  ...
  odio-upgrade apply [--version VERSION] [--state PATH] [--dry-run]
      ...
      the target release into bash. --reinstall forces every role to run in
      full (no smart-upgrade skips, all first-install scaffold re-applied).
...
# delta vs the published manifest, regenerated by `odio-upgrade check`,
# so it stays in /var/cache.
SYSTEM_STATE_PATH = "/var/lib/odio/state.json"
LEGACY_CACHE_STATE_PATH = "/var/cache/odio/state.json"
...
def find_state() -> tuple[str | None, StateLegacy | None, str | None]:
    ...
    Tries the canonical /var/lib path first, then the 2b1-era /var/cache
    location (next write_state.yml run migrates it forward), then the
    pre-2b1 per-user path of the *invoking* user — never globs /home/* so
    we don't need root. Returns (None, None, None) when the caller must
    rebuild from dpkg.
    """
    ...
    if os.path.isfile(LEGACY_CACHE_STATE_PATH):
        state = _read_state_file(LEGACY_CACHE_STATE_PATH)
        return LEGACY_CACHE_STATE_PATH, state, state.get("target_user")

    user_state = os.path.expanduser("~/.cache/odio/state.json")
    if os.path.isfile(user_state):
        # Pre-2b1 wrote state.json into target_user's own home, so the
...
def run_apply(opts: ApplyOptions) -> int:
    ...
    if not opts.force and not opts.reinstall and not opts.version \
